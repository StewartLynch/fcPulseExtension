...
FIRSTCLASS.fcp = {
	UNIXTOMAC: ((60*60*24*365*66)+(60*60*24*17)),
	saTimeToNative: function(saTime) {
		if (saTime == 0)
			return 0;        
		if (saTime < this.UNIXTOMAC)
			return 1;
		return saTime - this.UNIXTOMAC;
	},
	nativeToSATime: function(nativeTime) {
		return nativeTime + this.UNIXTOMAC;
	},
	jsUTCTimeToSaTime: function(jsDate){
		var saTime = 0;
		if ( jsDate )
			saTime = FIRSTCLASS.fcp.nativeToSATime(Math.ceil(jsDate.valueOf()/1000));
		return saTime;	
	},
	jsLocalTimeToSaTime: function(jsDate){
		var saTime = 0;
		if ( jsDate ) {
			var tzOffset = jsDate.getTimezoneOffset()
			if (tzOffset){
				tzOffset = tzOffset*60*1000;
				saTime = FIRSTCLASS.fcp.nativeToSATime(Math.ceil((jsDate.valueOf()-tzOffset)/1000));
			}
		}
		return saTime;
	},
	saTimeToJSUTCTime: function(saTime){
		var returnDate = undefined;
		if ( typeof(saTime.getMonth) == "function" )
			return saTime;
		if ( typeof(saTime) == "string" )
			saTime = parseInt(saTime,10);
		if ( typeof(saTime) == "number" ) {
			if (saTime < 0)
				saTime = saTime >>> 32;
			var tempDate = new Date(FIRSTCLASS.fcp.saTimeToNative(saTime)*1000);
			if (tempDate)
				returnDate = tempDate;
		}
		return returnDate;
	},
	saTimeToJSLocalTime: function(saTime){
		var returnDate = undefined;
		if ( typeof(saTime.getMonth) == "function" )
			return saTime;
		if ( typeof(saTime) == "string" )
		...
		if ( typeof(saTime) == "number" ) {
			...
			var tempDate = new Date(FIRSTCLASS.fcp.saTimeToNative(saTime)*1000);
			if (tempDate){
				var tzOffset = tempDate.getTimezoneOffset() *60 *1000;
				returnDate = new Date(tempDate.valueOf() + tzOffset);
			}
		}
		return returnDate;
	}
...
/*
Copyright (C) 2011 OpenText Corporation
FirstClass Web Server - JavaScript
*/

FIRSTCLASS.util = {
	multiply_uint32: function (a, b) {
...
FIRSTCLASS.password = {
	encodePassword: function (Buf,safeEncode,Seed) {
		var i;
		var newPass ='';
		Seed = FIRSTCLASS.util.base64.decode(Seed,true);
		Buf =  String.fromCharCode(Buf.length)+ Buf;
		for (i=Buf.length; i<28; i++) {
			Buf = Buf + String.fromCharCode(Math.floor(Math.random() *255));
		}
		for (i=0; i<28; i++) {
			newPass = newPass + String.fromCharCode((Buf.charAt(i).charCodeAt(0)) ^ (Seed.charAt(i).charCodeAt(0)));
		}
		newPass = FIRSTCLASS.util.base64.encode(newPass,true);
		return safeEncode ? encodeURIComponent(newPass) : newPass;
	}
};
